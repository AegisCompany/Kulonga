...
import { GraduationCap, Calendar, BookOpen, Bell, ChevronRight } from 'lucide-react';
  ...
  return (
    <div className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard label="Minha Média" value="15.8" icon={GraduationCap} color="bg-brand-green" />
        <StatCard label="Frequência" value="94%" icon={Calendar} color="bg-brand-blue" />
        <StatCard label="Tarefas" value="3" icon={BookOpen} color="bg-brand-orange" />
        <StatCard label="Próxima Prova" value="2 dias" icon={Bell} color="bg-red-500" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
          <h3 className="font-bold text-lg mb-6 text-brand-green">Minha Frequência Mensal</h3>
          <div className="h-64 flex items-end justify-between gap-2 px-4">
            {[65, 85, 75, 90, 80, 95, 70].map((h, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-2">
                ...
                >
                  <div className="absolute inset-0 bg-brand-orange rounded-t-lg opacity-0 group-hover:opacity-100 transition-opacity" />
                </motion.div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
          ...
          <div className="space-y-4">
            {[
              { title: 'Matemática', time: '08:00', date: 'Sala 101', color: 'bg-brand-blue' },
              { title: 'Física', time: '10:30', date: 'Lab 1', color: 'bg-brand-green' },
              { title: 'Química', time: '14:00', date: 'Sala 202', color: 'bg-brand-orange' },
            ].map((event, i) => (
