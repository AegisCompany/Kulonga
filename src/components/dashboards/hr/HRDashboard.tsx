...
import { Users, Sun, Plus, Shield, ChevronRight } from 'lucide-react';
...
const HRDashboard: React.FC<HRDashboardProps> = ({ stats }) => {
  return (
    <div className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard label="Total Professores" value={stats?.teachers?.toString() || '0'} icon={Users} color="bg-brand-blue" />
        <StatCard label="Férias Pendentes" value="3" icon={Sun} color="bg-brand-orange" />
        <StatCard label="Novas Contratações" value="2" icon={Plus} color="bg-brand-green" />
        <StatCard label="Licenças Ativas" value="5" icon={Shield} color="bg-slate-700" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
          <h3 className="font-bold text-lg mb-6 text-brand-green">Frequência de Funcionários</h3>
