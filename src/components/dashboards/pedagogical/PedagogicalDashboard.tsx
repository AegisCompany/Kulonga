...
import { School as SchoolIcon, BookOpen, GraduationCap, Calendar, ChevronRight } from 'lucide-react';
...
const PedagogicalDashboard: React.FC<PedagogicalDashboardProps> = ({ stats, subjectsCount }) => {
  return (
    <div className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard label="Turmas" value={stats?.classes?.toString() || '0'} icon={SchoolIcon} color="bg-brand-blue" />
        <StatCard label="Disciplinas" value={subjectsCount?.toString() || '0'} icon={BookOpen} color="bg-brand-green" />
        <StatCard label="Média Geral" value="14.8" icon={GraduationCap} color="bg-brand-orange" />
        <StatCard label="Horários Pendentes" value="0" icon={Calendar} color="bg-slate-700" />
      </div>
